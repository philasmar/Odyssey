...
public class CameraFit : MonoBehaviour
{
	...
	public enum VecticalSnapMode { Top, Middle, Bottom }
	[SerializeField]
	private VecticalSnapMode _verticalSnapMode = VecticalSnapMode.Middle;
	//private VecticalSnapMode _prevVerticalSnapMode;

	public enum HorizontalSnapMode { Left, Center, Right }
	[SerializeField]
	private HorizontalSnapMode _horizontalSnapMode = HorizontalSnapMode.Center;
	//private HorizontalSnapMode _prevHorizontalSnapMode;
    ...
    public void OnValidate()
    {
        ...
        if(_aspectCamera != null && _aspectCamera.pixelHeight != 0 && _aspectCamera.pixelWidth != 0)
        {
			...
			//else if (_prevVerticalSnapMode != _verticalSnapMode)
			//{
			//	_prevVerticalSnapMode
			//}
    ...
    private void ChangeViewForOrthographicCamera()
	{
		...
		float aspectDifferenceInverted;
		switch (_adjustMode)
        {
            case Mode.FixedHeight:
            ...
            case Mode.Dynamic:
				...
				aspectDifferenceInverted = Mathf.Min(1f, mainAreaAspectRatio / screenAspect);

				_aspectCamera.orthographicSize = Mathf.Max(verticalSize, horizontalSize / screenAspect);

				SetScissorRectForAllModes(aspectDifference, aspectDifferenceInverted, _aspectCamera);
	...
	private void SetScissorRectForAllModes (float aspectDifference, float aspectDifferenceInverted, Camera camera)
	{
		if (aspectDifference < 1)
		{
			if (_verticalSnapMode == VecticalSnapMode.Top)
			{
				camera.ResetProjectionMatrix();
				camera.projectionMatrix = SetScissorRect(new Rect(0, aspectDifference * 0.5f - 0.5f, 1, 1), camera.projectionMatrix);
			}
			else if (_verticalSnapMode == VecticalSnapMode.Bottom)
			{
				camera.ResetProjectionMatrix();
				camera.projectionMatrix = SetScissorRect(new Rect(0, 0.5f - aspectDifference * 0.5f, 1, 1), camera.projectionMatrix);
			}
			else //if (_prevVerticalSnapMode != _verticalSnapMode)
			{
				camera.ResetProjectionMatrix();
		...
		if (aspectDifferenceInverted < 1)
		{
			if (_horizontalSnapMode == HorizontalSnapMode.Right)
			{
				camera.ResetProjectionMatrix();
				camera.projectionMatrix = SetScissorRect(new Rect(aspectDifferenceInverted * 0.5f - 0.5f, 0, 1, 1), camera.projectionMatrix);
			}
			else if (_horizontalSnapMode == HorizontalSnapMode.Left)
			{
				camera.ResetProjectionMatrix();
				camera.projectionMatrix = SetScissorRect(new Rect(0.5f - aspectDifferenceInverted * 0.5f, 0, 1, 1), camera.projectionMatrix);
			}
			else
			{
				camera.ResetProjectionMatrix();
	...
	private Matrix4x4 SetScissorRect (Rect targetScissor, Matrix4x4 initialCameraProjectionMatrix)
	{
		Matrix4x4 scissorMatrix;

		Matrix4x4 m2 = Matrix4x4.TRS(new Vector3((1 / targetScissor.width - 1), (1 / targetScissor.height - 1), 0), Quaternion.identity, new Vector3(1 / targetScissor.width, 1 / targetScissor.height, 1));
		Matrix4x4 m3 = Matrix4x4.TRS(new Vector3(-targetScissor.x * 2 / targetScissor.width, -targetScissor.y * 2 / targetScissor.height, 0), Quaternion.identity, Vector3.one);
		scissorMatrix = m3 * m2 * initialCameraProjectionMatrix;
